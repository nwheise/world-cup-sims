...
| `cheer_guide.py` | **The cheering guide.** Consumes `preferences.json` weights and ranks all 72 group games by how much each result swings your odds of seeing a favorite in Seattle. Objective = *get my single favorite there*: per-sim `utility = max(weight of any Seattle team)`. Method: one big MC recording every scoreline + utility, then bucket each game by outcome (A-win/draw/B-win) and compare mean utility — valid as causal effect because group games are independent given ratings. Self-calibrating noise floor (3σ) hides games swamped by sampling error. Run: `python3 cheer_guide.py [n_sims]` (default 50000, seed=42). |
...
swamped by sampling error. (Possible future tweak: weight M94 above M82 since the R16 match is the marquee one.)
