...
| `.github/workflows/site.yml` | One workflow: push to main → tests + Pages deploy; **hourly cron** (`23 * * * *`; GitHub cron is best-effort — 30–90 min delays/skips observed, hence hourly) / manual dispatch → refresh results.json, commit if changed, tests + deploy. Scheduled/manual deploys stamp `checked_at` into the DEPLOYED results.json only (committed file keeps `fetched_at` = last actual score change; no churn commits) — the status line shows "last new result … · checked …". If `update_results.py` exits non-zero (a structural feed anomaly, not a transient outage) the refresh step records it and a final step fails the run *after* deploying, so GitHub's Actions failure notification emails us. Single workflow on purpose: GITHUB_TOKEN pushes don't trigger other workflows. |
