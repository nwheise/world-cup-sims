...
| `cheer_guide.py` | **The cheering guide.** Consumes `preferences.json` weights and ranks all 72 group games by how much each result swings your odds of seeing a favorite in Seattle. Objective = maximize the emphasized summed weight of your Seattle lineup (see workflow section). Method: one big MC recording every scoreline + utility, then bucket each game by outcome (A-win/draw/B-win) and compare mean utility — valid as causal effect because group games are independent given ratings. Self-calibrating noise floor (3σ) hides games swamped by sampling error. **Loyalty guard**: also buckets P(reach Seattle \| outcome) for each participant AND every liked team (weight ≥ `LIKE_FLOOR`, top 10, exposed as `p_lineup`); when the utility-max advice is to root *against* a team you like, the call is kept only if SOME liked team gains ≥ `SWING_THRESHOLD` — the denied team itself (`kind="self"`, their 3rd-place route) or a different favorite (`kind="other"`, e.g. a draw that denies Canada but protects Switzerland's group-winner path); otherwise (`kind="none"`) it's demoted to a footnote — so you're never told to cheer against a favorite for a payoff that isn't there. Worth-it calls are annotated with the honest swing and beneficiary. Helper `assess_loyalty()` is the pure, tested core. Run: `python3 cheer_guide.py [n_sims]` (default 50000, seed=42). |
...
buckets, per game and outcome, `P(reach Seattle | outcome)` for each participant and for every liked team
(weight ≥ `LIKE_FLOOR`, default 0.5; top 10, exposed per row as `p_lineup`). When the recommended result
roots against a team you like, it computes the **swing** in Seattle odds between the recommended result and
the result where the denied team wins — first for the denied team itself (`kind="self"`: their 3rd-place
route pays, e.g. `9% → 25% (+16pp)`), then, failing that, for every OTHER liked team (`kind="other"`: e.g. a
draw that denies Canada but lifts Switzerland's group-winner odds — the payoff can arrive via a different
favorite, which the original Seattle-only guard missed). If some liked team clears `SWING_THRESHOLD`
(default 0.03 = 3pp) the call stays, annotated with the honest payoff and beneficiary; if nobody you like
gains (`kind="none"`), it's demoted to a "LOYALTY NOTES" footnote (`just root for Mexico`) rather than
