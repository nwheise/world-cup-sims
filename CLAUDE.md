...
| `site/js/sim-core.js` | **The simulator + analysis engine** (pure ESM — identical in the Web Worker and under node tests). Official FIFA 2026 rules end to end; simulates all 104 matches, honoring real results (`prepareResults`): played group games pinned (keyed by team pair), known knockout winners attributed by their participants (not the m-number, so a mislabeled result id still lands on the right bracket slot) and honored only when they match the simulated lineup. Packs per-sim records (~216 bytes: 72 group outcomes, 48 group positions, 64 knockout participants, 32 winners) so re-analysis never re-simulates. Hosts `appearanceProbs` (per-slot + full joint matchup distributions + champion), `groupProbs`, `forcedKnockout` (per knockout slot, the team the bracket routes there in *every* sim, i.e. forced by the results so far even before FIFA publishes the fixture — the same "decided" signal the must-watch tab reads off a collapsed slot), `analyzeCheer` (cheer guide incl. loyalty guard, per-liked-team conditionals, top-picks summary), `analyzeTeamPath` (fan mode — see "The team-path math"), `assessLoyalty`, `emphasize`, `matchWDL` + `analyzeMatchCalibration` (per-match accuracy — see "The per-match-accuracy math"), and `resultsAsOf` (the **time-machine filter** — see "The time-machine"). Constants: `EMPHASIS=2.0`, `LIKE_FLOOR=0.5`, `SWING_THRESHOLD=0.03`, `CALIB_BINS` (reliability-diagram bins), seed 42, fixed 100k sims. |
| `site/js/worker.js` | Web Worker wrapper: `simulate` once (progress events), then `analyze` (weights + attended matches) and `teampath` (followed team) requests are instant re-aggregations. After simulating it runs `forcedKnockout` and merges both-known forced matchups into `fixed.koKnown` (so the cheer guide and team path bucket a knockout game the moment its lineup locks, no `results.json` knockout entry needed) and serializes the per-side resolution as `meta.koSlots` (id → `[name|null, name|null]`) for the display layer. This never re-runs the sim — the store is built and `simulateTournament` attributes recorded winners by participants (`koWinnerByParticipants`), not `koKnown` — so all probabilities are unchanged. |
...
| `site/data/results.json` | **Live results** (committed; final for the completed tournament — was refreshed by the now-removed hourly cron): `group_results` keyed `"TeamA\|TeamB"` (90-min scores), `knockout` keyed `m73..m104` (participants once known; score + winner once played). |
...
| `scripts/update_results.py` | Live results → results.json from the **ESPN public scoreboard API** (keyless, near-live; group vs knockout decided by the team pair, not the kickoff time — a real group fixture is a group game, any other pair of real teams is a knockout game. The knockout match number comes from `assign_knockout_slots`: because the feed carries every knockout match and the running order is fixed, it aligns the two chronological sequences one-to-one, so a delayed kickoff (even a whole session pushed back hours) can't mislabel a game; a partial feed falls back to nearest-kickoff. Winner from ESPN's per-competitor flag so ET/pens are correct). A transient ESPN outage logs a warning, leaves results.json untouched and exits 0 (next run retries); a structural surprise that won't parse exits non-zero so the workflow fails and GitHub notifies. Idempotent; no-op after Aug 1, 2026. `parse_espn()` / `assign_knockout_slots()` are pure and tested. |
...
| `.github/workflows/site.yml` | One workflow: push to main (or manual dispatch) → tests + Pages deploy. Stamps commit-SHA cache-busting versions onto asset URLs in the deployed copy only (repo files stay clean). The tournament is over, so the old **hourly cron** that refreshed results.json from ESPN has been removed; results.json is now a static committed file. |
...
- **Live mode**: played group games are pinned to real scores (keyed by team pair); known
  knockout winners are attributed by their two participants — not by the results.json
  m-number — so a live-feed match-id mislabel (e.g. from a kickoff change) still lands the
  result on the bracket slot those teams actually meet at, and it's honored only when it
  matches the simulated lineup (guards against partial/stale feeds). Only the remainder is
  simulated.
...
- **Knockout rows populate as lineups lock**: a knockout match is scored once BOTH its
  participants are known — either pinned in `results.json` or forced by the results so far
  (`forcedKnockout`, merged into `fixed.koKnown` by the worker). So a R32 game appears the
  moment both its feeding groups (and, for a third-place slot, the Annex C allocation)
  resolve to single teams, no FIFA-published fixture required. Until then it's not a real
  matchup, so it can't be on the path.
...
- **Documentation is part of the change, not a follow-up — keep it in sync proactively, on
  ...
  `EMPHASIS`, and the seed; the ratings source; the live-results source; the tab/feature
  list; the file layout; the rules) are described in four places
