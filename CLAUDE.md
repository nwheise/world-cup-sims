...
| `scripts/update_results.py` | Live results → results.json from the **ESPN public scoreboard API** (keyless, near-live; group vs knockout decided by the team pair, not the kickoff time — a real group fixture is a group game, any other pair of real teams is a knockout game whose match number is read off the nearest scheduled kickoff (they are ≥3.5h apart, so kickoff drift can't confuse them); winner from ESPN's per-competitor flag so ET/pens are correct). On an ESPN failure it logs a warning and leaves results.json untouched (the next run retries); one unplaceable game is skipped rather than aborting the whole refresh. Idempotent; no-op after Aug 1, 2026. `parse_espn()` is pure and tested. |
