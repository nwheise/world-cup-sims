...
6. **📊 Probabilities** — preference-independent forecasts: per-slot odds for every knockout
   ...
   top.
7. **📈 Accuracy** — how well-calibrated the model's per-match predictions are: a reliability
   diagram bins the win/draw/loss calls against real results (do the ~60% calls happen ~60%
   of the time?), with an overall Brier score. One tournament with correlated outcomes, so
   it's descriptive rather than a verdict.
